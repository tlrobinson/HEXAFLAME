...
import "./styles.css";
      ...
      function drawStepperGlow(currentScene) {
        const intensity = Math.min(Math.max(stepperPositionPercent / 100, 0), 1);
        if (intensity <= 0.001) {
          return;
        }
      ...
      function drawOutlineGlow(currentScene) {
        const outlineJets = currentScene.jets.filter(
          (jet) => jet.kind === "outline" && activeNodes.has(jet.controllerId),
        );
...
dfs sort shell-angle`,
      ...
      function render() {
        ...
        drawOutlineGlow(scene);
        drawStepperGlow(scene);
